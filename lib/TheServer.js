  ...
  defineControllerCreator (ControllerClass, as = ControllerClass.name) {
    ...
    return function controllerCreator (app, client) {
      ...
      const onClose = ({key}) => {
        ...
        ;(async () => {
          await target.controllerWillUnload && target.controllerWillUnload()
          await target.saveSession && target.saveSession()
          target = null
          emit = null
        })()
      }
      ...
      target.controllerDidLoad && target.controllerDidLoad()
      return creator
    }
  }
