...
class TheServer extends SugoHub {
  constructor (config = {}) {
    ...
    this.loadFromMapping(controllers)
  }
   ...
   */
  loadFromMapping (ControllerMapping) {
  ...
  defineControllerCreator (ControllerClass, as = ControllerClass.name) {
    ...
    return function controllerCreator (app, client) {
      ...
      const excludePatterns = [/^controllerWill/, /^controllerDid/, /^controllerMethod/]
      const controller = Object.keys(methods)
        .filter((action) => !excludePatterns.some((pattern) => pattern.test(action)))
        ...
        .reduce((bound, action) => Object.assign(bound, {
          [action]: async function controllerActionProxy (...params) {
            ...
            target.$emitter = this.$emitter || target.$emitter
            const invocation = new Invocation({target, action, params})
            return await invokeControllerAction(cid, invocation, {before, after, rescue})
          }
        }), {})
      if (connection) {
   ...
   * @type {string[]} Name of controllers
   */
  get knownControllerNames () {
