...
class RpcActor extends SugoActor {
  constructor ({ scope, onClient }) {
    ...
    s.on(CallerEvents.JOIN, ({ caller, messages }) => {
      let { key: callerKey } = caller
      let { scope } = messages || {}
      onClient && onClient(callerKey, scope)
    })
  }
...
module.exports = (scope, onClient) => new RpcActor({ scope, onClient })
