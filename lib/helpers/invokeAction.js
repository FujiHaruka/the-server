...
async function invokeAction (invocation, options = {}) {
  ...
  try {
    ...
    await target.controllerMethodWillInvoke && target.controllerMethodWillInvoke(action, params)
    invocation.result = await Promise.resolve(
      target[action](...params)
    )
    await target.controllerMethodDidInvoke && target.controllerMethodDidInvoke(action, params, invocation.result)
  } catch (error) {
