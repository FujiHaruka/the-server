...
class TheServer extends SugoHub {
  constructor (config = {}) {
    ...
    let rpc = rpcActor(appScope, (callerKey, scope) => co(function * () {
      ...
      let { cid } = scope
      let session = yield sessionStore.get(cid)
      let $$callerKeys = session ? session.$$callerKeys : []
      $$callerKeys.push(callerKey)
      yield sessionStore.set(cid, { $$callerKeys })
    }))
    super({
      storage: {
  ...
  defineControllerCreator (ControllerClass, as = ControllerClass.name) {
    ...
    return function controllerCreator (app, client) {
      ...
      Object.assign(target, {
        ...
        emit (event, data) {
          return co(function * () {
            let $$callerKeys = target.$$callerKeys || (yield s.sessionStore.get(cid)).$$callerKeys
            target.$$callerKeys = $$callerKeys
