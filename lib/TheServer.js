...
class TheServer extends SugoHub {
  constructor (config = {}) {
    ...
    let rpc = rpcActor(appScope, (callerKey, scope) => {
      const { sessionStore } = s
      let { cid } = scope
      sessionStore && sessionStore.set(cid, { $$callerKey: callerKey })
    })
    super({
      storage: {
  ...
  defineControllerCreator (ControllerClass, as = ControllerClass.name) {
    ...
    return function controllerCreator (app, client) {
      ...
      Object.assign(target, {
        ...
        emit (event, data) {
          return co(function * () {
            let callerKey = target.$$callerKey || (yield s.sessionStore.get(cid)).$$callerKey
