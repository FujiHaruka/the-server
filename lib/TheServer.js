...
class TheServer extends SugoHub {
  constructor (config = {}) {
    ...
    const rpc = rpcActor(appScope, {
      onClientJoin: (callerKey, scope) => s.createCallerConnection(callerKey, scope),
      onClientLeave: (callerKey) => s.destroyCallerConnection(callerKey)
    })
    super({
      storage: {
      ...
      teardown () {
        s.connectionPool.destroyAllConnections()
      },
      endpoints,
      logFile,
      keys
    ...
    s.connectionPool = new ConnectionPool()
  ...
  defineControllerCreator (ControllerClass, as = ControllerClass.name) {
    ...
    return function controllerCreator (app, client) {
      ...
      const connection = callerKey && s.connectionPool.getConnection(callerKey)
      ...
      const target = new ControllerModule({app, client, name: as})
      const {emit} = target
  ...
  createControllerFor (controllerName, app, client) {
    const s = this
    const creator = s.controllerCreators[controllerName]
    if (!creator) {
      const known = s.knownControllerNames()
  ...
  createCallerConnection (callerKey) {
    const s = this
    s.connectionPool.createConnection(callerKey)
  }

  destroyCallerConnection (callerKey) {
    const s = this
    s.connectionPool.destroyConnection(callerKey)
  }
